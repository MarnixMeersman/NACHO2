...
index = 0
observations = []
x_earths, y_earths, z_earths, x_asteroids, y_asteroids, z_asteroids = [], [], [], [], [], []
...
with open(write_path, 'w') as f:
    ...
    for asteroid in asteroid_ephemerides:
        ...
        x_earths.append(np.array(earth[0]))
        y_earths.append(np.array(earth[1]))
        z_earths.append(np.array(earth[2]))
        x_asteroids.append(np.array(asteroid['eph'][1]))
        y_asteroids.append(np.array(asteroid['eph'][2]))
        z_asteroids.append(np.array(asteroid['eph'][3]))
...
x_earths = np.array(x_earths).flatten()
y_earths = np.array(y_earths).flatten()
z_earths = np.array(z_earths).flatten()

x_asteroids = np.array(x_asteroids).flatten()
y_asteroids = np.array(y_asteroids).flatten()
z_asteroids = np.array(z_asteroids).flatten()
